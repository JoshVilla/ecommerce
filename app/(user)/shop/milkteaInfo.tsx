...
const MilkteaInfo = ({ record }: Props) => {
  ...
  const [btnSugarLevelActive, setBtnSugarLevelActive] = useState<number | null>(null);
  ...
  const totalAmount = useMemo(() => {
    if (order) {
      const subtotal = Object.values(order).reduce(
          (sum: number, value: any) => (typeof value === "number" ? sum + value : sum),
          0
      );
  ...
  const addToCart = () => {
    ...
    const finalOrder = {
      ...
      total: totalAmount,
      product: record.name,
      description: `Size: ${order.size}, Addons: ${order.addon || "None"}, Sugar: ${order.sugar}`,
  ...
  return (
      ...
      >
        <SheetTrigger asChild>
          <Button size="sm" variant="default" onClick={() => setOpenSheet(true)}>
            <ShoppingCart />
          </Button>
        </SheetTrigger>
        <SheetContent>
